...
- `bot/config.py` — loads `TELEGRAM_BOT_TOKEN`, 4× `DRIVER_GROUP_*`, `BRAND_GROUP`, and optional `ARCHIVE_GROUP` from env
...
- `bot/handlers/brand.py` — branding application: eligibility warning → name → phone → model → year → color → plate. Sends the application to the brand group.
